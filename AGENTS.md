...
- **Errors.** Data sources throw the exceptions in `lib/core/error/exceptions.dart`. Repositories wrap work in `Result.guard` and return `Result<T>` (`Ok` / `Err` with a sealed `Failure`). Callers unwrap it with an exhaustive `switch`. `Error`s (bugs) are never caught. Blocs keep the `Failure` in their state; widgets render it with `failure.localizedMessage` (`lib/core/error/failure_message.dart`).
